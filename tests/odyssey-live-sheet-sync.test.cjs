...
test('live sheet sync promotes the full current 309 and 584-art library',()=>{
 const d=sync.apply(baseline());
 assert.equal(d.datasetVersion,'2026-09-26.4');
 assert.equal(d.cards.length,309);assert.equal(d.artworks.length,584);assert.equal(d.coverage.length,309);
 assert.equal(new Set(d.cards.map(c=>c.number)).size,309);assert.equal(new Set(d.artworks.map(a=>a.id)).size,584);
 ...
 assert.equal(assigned.length,303);for(const c of assigned)assert.ok(art.has(c.primaryArt),c.id+' has current art');
 assert.deepEqual(d.cards.filter(c=>!c.primaryArt).map(c=>c.number),[131,135,259,262,263,266]);
});
...
test('the newest artwork wave is directly renderable and assigned',()=>{
 ...
 assert.equal(fresh.length,31);assert.ok(fresh.every(a=>/^https:\/\//.test(a.imageUrl)),fresh.filter(a=>!a.imageUrl).map(a=>a.id).join(','));
...
test('Studio loads the sheet sync before model construction and links the current card source',()=>{
 ...
 const data=app.indexOf('odyssey-data.js?v=20260926-closing2'),syncPos=app.indexOf('live-sheet-sync.js?v=20260927-art2'),model=app.indexOf('const ODYSSEY_DATASET=loadOdysseyDataset()');
 assert.ok(data>=0&&syncPos>data&&model>syncPos);
 assert.match(app,/1-OTRpW8vrSJWcXcL06l3eMJESwFtt6hQqCEl9J3sdEE\/edit/);
 const index=fs.readFileSync(path.join(root,'public/mtgtools/odyssey/index.html'),'utf8');assert.match(index,/app\.html\?v=20260927-art2/);
});
