...
test('live sheet sync promotes the full current 309 and 582-art library',()=>{
 const d=sync.apply(baseline());
 assert.equal(d.datasetVersion,'2026-09-26.3');
 assert.equal(d.cards.length,309);assert.equal(d.artworks.length,582);assert.equal(d.coverage.length,309);
 assert.equal(new Set(d.cards.map(c=>c.number)).size,309);assert.equal(new Set(d.artworks.map(a=>a.id)).size,582);
...
test('the newest artwork wave is directly renderable and assigned',()=>{
 ...
 assert.equal(fresh.length,29);assert.ok(fresh.every(a=>/^https:\/\//.test(a.imageUrl)),fresh.filter(a=>!a.imageUrl).map(a=>a.id).join(','));
