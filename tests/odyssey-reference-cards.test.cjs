...
test('every current Odyssey card has a refreshed real-card reference set',()=>{
  ...
  for(const c of current){
    const e=refs.cards[c.id];
    assert.ok(e,c.id);
    assert.equal(e.number,c.number);
    assert.ok(e.references.length>=1&&e.references.length<=7,c.id+' '+e.references.length);
    if(!/\bBasic Land\b/.test(c.type||'')){
      const roles=new Set(e.references.map(r=>r.role));
      assert.ok(roles.has('rate-best'),c.id+' missing best-rate');
      assert.ok(roles.has('rate-normal'),c.id+' missing normal-rate');
    }
  }
});

test('quest-counter progression cards carry the dedicated current quest-tech precedent',()=>{
  const quests=current.filter(c=>/quest counter/i.test((c.rules||'')+' '+(c.mechanics||'')));
...
test('reference cards are distinct released Scryfall cards with imagery and annotations',()=>{
  for(const e of Object.values(refs.cards)){
    ...
    for(const r of e.references){
      ...
      assert.ok(typeof r.card.oracleText==='string',e.id+' '+r.card.name+' oracle text field');
      if(!/\bLand\b/.test(r.card.type||''))assert.ok(r.card.oracleText.trim(),e.id+' '+r.card.name+' oracle text');
      assert.ok(r.card.releasedAt<=refs.scryfallBulk.releasedOnOrBefore,e.id+' unreleased reference '+r.card.name);
      const key=r.card.oracleId||r.card.id;
      assert.ok(!seen.has(key),e.id+' duplicate '+r.card.name);
      seen.add(key);
    }
  }
});

test('annotations expose costing, mechanic and splashy-design lenses',()=>{
  ...
  assert.ok(notes.some(x=>/^Build-around \/ interesting spin:/.test(x)));
});

test('rare and mythic originals get an expanded comparison set',()=>{
  const splashy=current.filter(c=>['R','M'].includes(String(c.rarity||''))&&!/\bBasic Land\b/.test(c.type||''));
  const expanded=splashy.filter(c=>refs.cards[c.id].references.length>=5);
  assert.ok(expanded.length>=Math.floor(splashy.length*.8),expanded.length+'/'+splashy.length);
});

test('Studio loads v2 reference data before the review browser and keeps compact cards to three',()=>{
  ...
  for(const phrase of ['Real card references','Best rate','Normal rate','Set / mechanic tech','Build-around / spin','refs.slice(0,3)','odyssey-reference-review-v1'])assert.ok(browser.includes(phrase),phrase);
});

test('release metadata records the exact reference snapshot and policy',()=>{
  const release=JSON.parse(fs.readFileSync(path.join(dir,'data/release.json'),'utf8'));
  assert.equal(release.referenceCards.schema,'odyssey-reference-cards/v2');
  ...
  assert.match(release.referenceCards.policy,/best-rate/);
});
