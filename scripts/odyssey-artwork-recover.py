"""Repair the first source audit without substituting unrelated works or altering card data."""
...
# One catalogued 97.1 MiB TIFF is explicitly allowed; output still has the 25 MiB cap.
...
notes={}
# Official NGA published image table links exactly to the numeric collection object IDs.
nga={111489:'ART-143',151069:'ART-145',45099:'ART-165',143861:'ART-170',157097:'ART-177',44055:'ART-191',221932:'ART-192',69829:'ART-424'}
try:
 raw,_,_=a.get('https://raw.githubusercontent.com/NationalGalleryOfArt/opendata/main/data/published_images.csv')
 matches=[]
 for rawrow in csv.DictReader(io.StringIO(raw.decode('utf-8-sig'))):
...
except Exception as e:notes['ngaError']=str(e)
# The Rijks object was stored as its persistent numeric ID, not its inventory number.
...
notes['ART-421']={'source':'Smithsonian Open Access reproduction on Wikimedia Commons','objectId':'1901-39-1640','commonsFile':'Drawing, Zeus Deciding on the Destiny of Troy, Palazzo Gessi, Faenza, 1813 (CH 18110481).jpg'}
# Preserve a truthful unavailable record if the collection itself publishes no image.
for art in a.DATA['artworks']:
 if art['id']=='ART-200':
  try:
...
manifest['summary']=summary;manifest['checkedAt']=time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime())
manifest['revision']='artwork-performance-v1'
