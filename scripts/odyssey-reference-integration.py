...
idx=APP/'index.html';x=idx.read_text();x=re.sub(r'app\\.html\\?v=[^\"\']+','app.html?v=20260922-ref2',x);idx.write_text(x)
pol=APP/'studio-polish.js';x=pol.read_text().replace('v3.21 · full authoring','v3.22 · card references');pol.write_text(x)
release=APP/'data/release.json';d=json.loads(release.read_text());d['version']='2026-09-22.4';d['revision']='14A2-ff-lattice-rebuild+studio-notes-20260922-v1+references-v2';d['appRevision']='14A2-reference-rebuild-v2';d['referenceCards']={'schema':refs['schema'],'cards':refs['totalCards'],'totalReferences':sum(len(v['references']) for v in refs['cards'].values()),'roles':dict(__import__('collections').Counter(r['role'] for v in refs['cards'].values() for r in v['references'])),'scryfallBulkUpdatedAt':refs['scryfallBulk'].get('updatedAt'),'source':'Scryfall Oracle Cards bulk data','policy':'best-rate + normal-rate + mechanic/set-tech + template + splashy build-around'};release.write_text(json.dumps(d,ensure_ascii=False,separators=(',',':'))+'\n')
