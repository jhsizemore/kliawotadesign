...
quotes=[e['flavour']['candidate'] for e in editorial_cards.values() if e.get('flavour',{}).get('status')=='candidate-verified' and e['flavour'].get('candidate')]
...
for b in briefs:
 ...
 e=editorial_cards.get(b['id'],{})
 if e.get('flavour',{}).get('status')=='no-direct-quote-selected':
...
template=template.replace('__CHANGE_COUNT__',str(len(changes))).replace('__QUOTE_COUNT__',str(len(quotes)))
