'use strict';
let pvGraphics=null,pvWaterFrame=0,pvEnvRT=null,pvSkyTexture=null,pvSun=null;
...
function wrapRefresh(){
  if(typeof refreshAll!=='function'||refreshAll._pivot2)return;const prior=refreshAll;refreshAll=function(){prior();tuneWorldMaterials();};refreshAll._pivot2=true;
}
...
function updatePivotUI(){
  document.title='Port Vila Sandbox — Visual Pivot 2';
  const badge=document.querySelector('.title b');if(badge)badge.textContent='VISUAL PIVOT 2';
  const label=document.querySelector('aside .label');if(label)label.textContent='VISUAL PIPELINE · STAGE 2';
  const h=document.querySelector('aside h2');if(h)h.textContent='Tropical daylight + PBR environment';
  const p=document.querySelector('aside p');if(p)p.textContent='A generated tropical daylight environment now lights the physical materials, with stronger directional shadows, atmospheric depth and tuned surfaces across the same mapped Port Vila world.';
  const loadH=document.querySelector('.loadcard h1');if(loadH)loadH.textContent='Lighting the living city…';
  const hint=document.querySelector('.hint');if(hint)hint.textContent='© OpenStreetMap contributors · Terrain Tiles / Mapzen on AWS · procedural daylight environment + PBR';
}
...
async function init(){
  ...
  ['terrain','urban','roads','buildings','labels','shore','waterFX'].forEach(group);makePhysicalWater();wrapRefresh();bindControls();bindUI();cameraUpdate();startPivotLoop();
  Promise.resolve(load()).finally(()=>{tuneWorldMaterials();const n=$('idstat')?.textContent||'—';$('status').textContent=`Pivot 2 · ${rendererLabel()} · ${pvGraphics.name}${n!=='—'?` · ${n} identified`:''}`});
}
