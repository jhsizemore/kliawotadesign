...
function tile(ref,compact){
 ...
 "<div class='od-ref-copy'><div class='od-ref-head'><span class='od-ref-role "+roleClass(ref.role)+"'>"+esc(roleLabel(ref.role))+"</span><strong>"+esc(c.name||"Reference")+"</strong></div>"+
 "<div class='od-ref-meta'>"+esc(c.manaCost||"")+" · "+esc(c.type||"")+" · "+esc((c.rarity||"").toUpperCase())+"</div>"+
 "<p>"+esc(ref.annotation||"")+"</p>"+(compact?"":(c.oracleText?"<div class='od-ref-oracle'><b>Oracle text</b><br>"+esc(c.oracleText).replace(/\\n/g,"<br>")+"</div>":""))+(tags?"<div class='od-ref-tags'>"+tags+"</div>":"")+
...
function renderModal(){
 ...
 overlay.querySelector("[data-ref-progress]").textContent=c.done+"/"+c.total+" original designs reviewed";
 overlay.querySelector("[data-ref-grid]").innerHTML=refs.length?refs.map(function(r){return tile(r,false)}).join(""):"<div class='gallery-empty'>This card has no original-design reference set.</div>";
 var mark=overlay.querySelector("[data-ref-mark]");mark.disabled=!refs.length;mark.textContent=isReviewed(selected)?"Reviewed ✓":"Mark reviewed + next"
}
...
var api={entryFor:entryFor,refsFor:refsFor,counts:counts,open:open,close:close,mount:mount};if(typeof module!=="undefined"&&module.exports)module.exports=api;root.OdysseyReferenceBrowser=api;if(typeof document!=="undefined"){if(document.readyState==="complete")setTimeout(mount,0);else root.addEventListener("load",function(){setTimeout(mount,0)},{once:true})}
